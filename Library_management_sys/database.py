...
def get_borrowers(id=None):
    ...
    borrowers = [{'borrower_id': row[0], 'name': row[1], 'email': row[2], 'bid': row[3]} for row in rows]
...
def add_bor(name,email,bid):
    cursor = connection.cursor()
    cursor.execute("INSERT INTO borrowers (name, email, bid) VALUES (?, ?, ?)", (name,email,bid))
    connection.commit()
...
def update_bor(id, name, email, bid):
    cursor = connection.cursor()
    cursor.execute(f"update borrowers set bid={bid},name='{name}',email='{email}' where borrower_id={id}")
    connection.commit()    
...
def search_book(title):
    cursor = connection.cursor()
    cursor.execute(f"SELECT borrowers.borrower_id, borrowers.name, borrowers.email,books.title,borrowers.bid FROM books JOIN borrowers ON books.book_id = borrowers.bid WHERE books.title = '{title}'")
    connection.commit()
